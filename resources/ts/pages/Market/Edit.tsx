...
import { db } from "@/db";
import { saveListLocally, deleteListLocally } from "@/sync";
...
export default () => {
  ...
  const [form, setForm] = useState<PurchaseList>(newPurchaseList());
  ...
  const getPurchaseList = async () => {
    ...
    try {
      const resp = await axios.get<PurchaseList>(`/api/purchase-lists/${id}`);
      setForm(resp.data);
      await db.purchaseLists.put(resp.data);
    } catch {
      const numId = parseInt(id);
      const dbKey: string | number = isNaN(numId) ? id : numId;
      let cached = await db.purchaseLists.get(dbKey);

      if (!cached && String(id).startsWith('offline_')) {
        const mapped = await db.tempIdMap.get(id);
        if (mapped) {
          cached = await db.purchaseLists.get(mapped.realId);
        }
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (String(submitForm.id).startsWith('offline_')) {
        const mapped = await db.tempIdMap.get(String(submitForm.id));
        if (mapped) {
          submitForm = { ...submitForm, id: mapped.realId };
        }
      }

      const resp = await axios.post<{ list: PurchaseList }>('/api/purchase-lists', {
        ...submitForm,
        id: (!submitForm.id || submitForm.id === '') ? undefined : submitForm.id,
      });

      const saved = resp.data.list;
      setForm(saved);
      await db.purchaseLists.put(saved);
      toasts.success('Lista salva com sucesso.');

      if (!form.id || form.id === '' || id === 'new') {
        router.visit(`/purchase-lists/${saved.id}`, { replace: true });
      }
    } catch (err: any) {
      if (!err.response) {
        const localId = await saveListLocally(form);
        toasts.success('Salvo localmente. Será sincronizado quando houver conexão.');
        if (!form.id || form.id === '' || id === 'new') {
          router.visit(`/purchase-lists/${localId}`, { replace: true });
        }
      } else if (err.response?.status === 422) {
        const raw = err.response.data.errors ?? {};
        const first: Record<string, string> = {};
        Object.keys(raw).forEach((k) => {
          first[k] = Array.isArray(raw[k]) ? raw[k][0] : raw[k];
        });
        setErrors(first);
      }
    } finally {
  ...
  const deleteItem = async () => {
    ...
    try {
      if (String(id).startsWith('offline_')) {
        await deleteListLocally(id);
      } else {
        try {
          await axios.delete(`/api/purchase-lists/${id}`);
          await db.purchaseLists.delete(parseInt(id) || id);
        } catch (err: any) {
          if (!err.response) {
            await deleteListLocally(id);
            toasts.success('Exclusão pendente. Será sincronizada quando houver conexão.');
          } else {
            setDeleting(false);
            return;
          }
  ...
  const getSumOfMarkedItems = () => {
    return form.items.reduce((acc, cur) => {
      if (cur.checked == 0 || cur.deleted) return acc;
      return acc + (cur.quantity ?? 0) * (cur.unitary_value ?? 0);
    }, 0);
  };

  const getBalance = () => form.budget - getSumOfMarkedItems();

  const [confirmationDeleteList, setConfirmationDeleteList] = useState(false);
  const [swipeStates, setSwipeStates] = useState<{
    [key: number]: { offset: number; showDelete: boolean; isDragging: boolean };
  }>({});
  ...
  const areSomeItemsChecked = () =>
    form.items.filter((item) => !item.deleted).some((item) => item.checked === 1);

  const updateSwipeState = (
    index: number,
    updates: Partial<{ offset: number; showDelete: boolean; isDragging: boolean }>
  ) => {
    setSwipeStates((prev) => ({
      ...prev,
      [index]: {
        ...{ offset: 0, showDelete: false, isDragging: false },
        ...prev[index],
        ...updates,
      },
    }));
  ...
  const handleSwipeStart = (index: number, _clientX: number) => {
  ...
  const handleSwipeMove = (index: number, clientX: number, startX: number) => {
    ...
    if (offset > 0) {
      ...
      updateSwipeState(index, { offset: -newOffset, showDelete: newOffset > 60 });
    }
  };
  ...
  return (
    <>
    <LoadingOverlay show={isLoadingForm} message="Carregando lista..." />
    <LoadingOverlay show={!isLoadingForm && processing} message="Salvando..." />
    <Layout
      header={
        <React.Fragment>
    ...
    >
      <div className="w-full sm:max-w-[500px] mx-auto flex flex-wrap pb-60">
        ...
        <div className="w-full mt-4 overflow-hidden relative">
          {Object.entries(swipeStates).map(([index, state]) => {
            ...
            const visibleIndex =
              form.items.filter((_, i) => i <= itemIndex && !form.items[i].deleted).length - 1;
            const rowHeight = 26;
            ...
            return (
              <div
                ...
                style={{ width: "100%", height: `${rowHeight}px`, top: `${topPosition}px` }}
              >
          ...
          <table className="w-full table list-table">
            ...
            <tbody>
              {form.items.map((item, originalIndex) => {
                ...
                const swipeState = swipeStates[originalIndex] || {
                  offset: 0,
                  showDelete: false,
                  isDragging: false,
                };
                const visibleIndex =
                  form.items.filter((_, i) => i <= originalIndex && !form.items[i].deleted)
                    .length - 1;
                  ...
                  >
                    ...
                    <td className="w-[17%] border text-left">
                      <InputDiv
                        ...
                        onInput={(event) =>
                          handleNumberChange(event, "unitary_value", originalIndex)
                        }
                        inputMode="decimal"
                      />
                    </td>
      ...
      <div className="w-full flex bg-white shadow-[15px_10px_10px_5px] p-2 justify-center fixed bottom-0 left-0">
